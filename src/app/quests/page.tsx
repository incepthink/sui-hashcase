      ...
      <div className="pt-32 pb-6">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-center mb-6">
            <h1 className="text-2xl font-bold text-white mb-2">Available Quests</h1>
            {mounted && isWalletConnected && !loading && (
              <div className="max-w-md mx-auto mb-6">
                ...
                <div className="w-full bg-gray-800 rounded-full h-3 overflow-hidden">
                  <div className="bg-white h-full rounded-full transition-all duration-500 ease-out" style={{ width: `${completionPercentage}%` }} />
                </div>
              </div>
            )}
          </div>
          ...
          <div className="space-y-2">
            {quests.map((quest) => (
              <div key={quest.id} className="group bg-gray-900 border border-gray-700 rounded-lg p-3 relative overflow-hidden">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-4">
                    <div className="flex flex-col">
                      <h3 className="text-base font-bold text-white">{quest.title}</h3>
                      <p className="text-gray-400 text-xs hidden md:block">{quest.description}</p>
                    </div>
                  </div>
                  <div className="flex items-center space-x-4">
                    {quest.is_completed ? (
                      <span className="text-xs text-green-400 bg-green-900/20 px-2 py-1 rounded border border-green-700">✓ Completed</span>
                    ) : !isWalletConnected ? (
