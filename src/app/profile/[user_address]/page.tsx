...
const App: React.FC = () => {
  ...
  const MY_PACKAGE_ID = "0xea46060a8a4750de4ce91e6b8a2119d35becbeaef939c09557d0773c7f7c20a0";
  ...
  console.log("🔍 DEBUG: Environment Package ID:", process.env.NEXT_PUBLIC_CONTRACT_PACKAGE_ID);
  console.log("🔍 DEBUG: Final Package ID:", MY_PACKAGE_ID);
  console.log("🔍 DEBUG: Raw owned objects:", myLoyaltyData?.data);
  console.log("🔍 DEBUG: All object types:", myLoyaltyData?.data?.map(item => item?.data?.type));
  console.log("🔍 DEBUG: Expected NFT type:", `${MY_PACKAGE_ID}::hashcase_module::NFT`);

  //filtering out the NFT types of object
  const filteredNFTs = myLoyaltyData?.data.filter(
    (item) => item?.data?.type?.includes('::hashcase_module::NFT')
  );
